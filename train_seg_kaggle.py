...
parser.add_argument('--trainDir', default="/data2/coldplay/dsb_cell/DataSets/kaggle/train", type=str, help='training image directory')
parser.add_argument('--valDir', default="/data2/coldplay/dsb_cell/DataSets/kaggle/val", type=str, help='validation image directory')
parser.add_argument('--annoDir', default="/data2/coldplay/dsb_cell/DataSets/kaggle/mask", type=str, help='annotation image directory')
...
parser.add_argument('--weightDst', default='/data2/coldplay/dsb_cell/seg_weights', type=str, help='weight save folder')
...
parser.add_argument('--dec_weights', default="/data2/coldplay/dsb_cell/dec_weights/end_model.pth", type=str, help='detection weights')
    ...
    for epoch in range(args.num_epochs):
        ...
        for phase in ['train', 'val']:
            if phase == 'train':
                ...
                np.savetxt('/data2/coldplay/dsb_cell/train_loss.txt', train_loss_dict, fmt='%.6f')
                writer.add_scalar('/data2/coldplay/dsb_cell/train',epoch_loss,epoch)
                if epoch % 5 == 0:
                ...
                writer.add_scalar('scalar/val/ap05',ap_05,epoch)
                writer.add_scalar('scalar/val/ap07',ap_07,epoch)
               # print('ap05:{:.4f}, ap07:{:.4f}'.format(ap_05, ap_07))
         
                ap05_dict.append(ap_05)
                np.savetxt('/data2/coldplay/dsb_cell/ap_05.txt', ap05_dict, fmt='%.6f')
                ap07_dict.append(ap_07)
                np.savetxt('/data2/coldplay/dsb_cell/ap_07.txt', ap07_dict, fmt='%.6f')
                if ap_05>70.9:
                    print(ap_05)
                    torch.save(seg_model.state_dict(),os.path.join(args.weightDst,'{:d}_{:.4f}_model.pth'.format(epoch,epoch_loss)))
